...
uint read_sample(FILE *fd, const imgInfo *img) {
	switch (img->depth) {
	case 8:
		return (ushort)fgetc(fd);
	case 16:
		return (ushort)(fgetc(fd)) << 8 | fgetc(fd);
	default:
...
int load_pam(const char *fname, imgInfo *img) {
	...
	size_t memSize = sizeof(img->data[0]);
	memSize *= img->channels + (img->channels & 1); /* add one channel if number of channels is odd */
	memSize *= img->width;
	memSize *= img->height;
	img->data_size = memSize;
	img->data = (ushort *)malloc(memSize);
	if (img->data == NULL) {
	...
	for (uint row = 0; row < img->width; ++row) {
		for (uint col = 0; col < img->height; ++col) {
			for (uint ch = 0; ch < img->channels; ++ch) {
				img->data[cur++] = read_sample(fp, img);
			}
			/* skip one */
			if (img->channels & 1)
				++cur;
